...
class DDPM(pl.LightningModule):
    ...
    def p_losses(self, x_start, t, noise=None):
        noise = default(noise, lambda: torch.randn_like(x_start))
        x_noisy = self.q_sample(x_start=x_start, t=t, noise=noise)#前向加噪，因为是确定的过程所以仅一步
...
class LatentDiffusion(DDPM):
    ...
    def __init__(self,
        ...
        self.cond_stage_model.adapter_train()
        self.cond_stage_model.adapter_activate()
        print("_______________adapter_train_summary__________________")        
        print(self.cond_stage_model.transformer.adapter_summary(as_dict=False))            
        print(self.cond_stage_model.transformer)   
    ...
    def p_losses(self, x_start, cond, t, noise=None):
        noise = default(noise, lambda: torch.randn_like(x_start))
        x_noisy = self.q_sample(x_start=x_start, t=t, noise=noise) #前向加噪，确定过程，所以仅一步
    ...
    def configure_optimizers(self):
        ...
        if self.embedding_manager is not None: # If using textual inversion
            ...
            assert self.cond_stage_model.adapter_parameters()
            if self.cond_stage_model.adapter_parameters() is not None:
                embedding_params = self.cond_stage_model.adapter_parameters()    
    ...
    def on_save_checkpoint(self, checkpoint):
        ...
        if os.path.isdir(self.trainer.checkpoint_callback.dirpath):
            self.cond_stage_model.adapter_save(os.path.join(self.trainer.checkpoint_callback.dirpath, "adapter"))
            self.cond_stage_model.adapter_save(os.path.join(self.trainer.checkpoint_callback.dirpath, f"adapter_gs-{self.global_step}"))                
