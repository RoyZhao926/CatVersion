...
class CLIPEvaluator(object):
    def __init__(self, device, clip_model='ViT-B/32', download_root='../clip_model') -> None:
        self.device = device
        self.model, clip_preprocess = clip.load(clip_model, device=self.device, download_root=download_root)
...
class CustomCLIPEvaluator(CLIPEvaluator):
    def __init__(self, device, clip_model='ViT-B/32', download_root='/data/roy/clip_model') -> None:
        super().__init__(device, clip_model, download_root)

    @torch.no_grad()
    def encode_images(self, images: Image) -> torch.Tensor:
        images = self.clip_preprocess(images).unsqueeze(0).to(self.device)
        return self.model.encode_image(images)
